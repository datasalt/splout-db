...
public class TestQuerier {
	...
	public void testAllFailingDNodes() throws JSONSerDeException {
		...
		Tablespace tablespace = new Tablespace(PartitionMap.oneShardOpenedMap(),
		    new ReplicationMap(rEntries), 0, 0);
	...
	@Test
	public void testNoPartitionMap() throws Throwable {
		SploutConfiguration testConfig = SploutConfiguration.getTestConfig();
		...
		DNode dnode1 = TestUtils.getTestDNode(testConfig, okQueryHandler, "dnode-"
		    + this.getClass().getName() + "-1");
		DNode dnode2 = TestUtils.getTestDNode(testConfig, okQueryHandler, "dnode-"
		    + this.getClass().getName() + "-2");

		ReplicationMap balancedMap = ReplicationMap.oneToOneMap(dnode1.getAddress(), dnode2.getAddress());

		QNodeHandlerContext context = new QNodeHandlerContext(testConfig, null);
		// Tablespace with null partition map
		// This means there is no explicit key ordering
		// So queries should be routed as key.hash % partitions
		Tablespace tablespace = new Tablespace(null, balancedMap, 0, 0);
		context.getTablespaceVersionsMap().put(new TablespaceVersion("t1", 0l), tablespace);
		context.getCurrentVersionsMap().put("t1", 0l);
		...
		for(int i = 0; i < 5; i++) {
			...
			QueryStatus status = querier.query("t1",partitionKey,"sql",null);
			// So we assert that the partition queried was hash % nPartitions = hash % 2
			assertEquals((Integer)(partitionKey.hashCode() % 2), status.getShard());
	...
	public void testRoundRobin() throws Throwable {
		...
		/*
		 * Here we are testing basic round robin: we have 2 nodes serving one shard and we do 5 queries so we want to assert
		 * that the node whom we are querying at step "i" is exactly "i % 2".
		 */
		for(int i = 0; i < 5; i++) {
	...
	public void testRoundRobinWithSomeDeadNodes() throws Throwable {
		...
		List<ReplicationEntry> rEntries = new ArrayList<ReplicationEntry>();
		rEntries.add(new ReplicationEntry(0, dnode1.getAddress(), "fakeaddress:1111", dnode2.getAddress(),
		    "fakeaddress:2222"));
		...
		/*
		 * Here we are testing round robin when there are some dead nodes in a shard. We have 2 alive nodes and 2 dead
		 * nodes. Therefore we want to assert that at step "i" the chosen node will be "0 if i % 2 == 0 or 2 if i % 2 == 1".
	...
	public void testRoundRobinWithSomeFailingNodes() throws Throwable {
		...
		List<ReplicationEntry> rEntries = new ArrayList<ReplicationEntry>();
		rEntries.add(new ReplicationEntry(0, dnode1.getAddress(), "failingaddress:1111",
		    dnode2.getAddress(), "failingaddress:2222"));
		...
		/*
		 * Here we are testing round robin when there are failing nodes in a shard. We have 4 alive nodes with 2 failing
		 * nodes. The assertion is the same than in the case of some dead nodes but we do another unit test because the
		 * business logic inside the querier is different and we might have a bug anyway.
		 */
		for(int i = 0; i < 7; i++) {
			QueryStatus status = querier.query("t1", "", 0);
			assertEquals((Integer) ((i % 2) * 2), querier.getPartitionRoundRobin().get(0));
