...
/**
 * Similar to {@link TestMultiThreadedQueryAndDeploy} but it only performs one
 * deploy at the beginning and then it queries and at the same time brings some
 * dNodes down and up... This is for checking parallel robustness of fail-over.
 */
public class TestMultiThreadedFailover extends BaseIntegrationTest {
          ...
          public void run() {
            try {
              while (true) {
                ...
                QueryStatus status = client.query(TABLESPACE, ((randomDNode * 10) - 1) + "", "SELECT * FROM " + TABLE + ";", null);
  ...
  private void deployIteration(int iteration, Random random, SploutClient client, Tablespace testTablespace) throws Exception {
    ...
    client.deploy(TABLESPACE, testTablespace.getPartitionMap(), testTablespace.getReplicationMap(), deployData.getAbsoluteFile().toURI());
  }
}
